...
def analyze_failed_logins(path: str, min_count: int):
    ...
    filtered_ips = Counter({ip: count for ip, count in ip_counter.items() if count >= min_count})

    print("Failed Login Attempts by IP")
    _print_counter(filtered_ips, "IP Address", "Attempts")
...
# ── TODO Task 3: fill this in ───────────────────────────────────────────────
    ...
    print(f"{'Username':<15} {'Password':<15} {'IP_Count':>8}")
    print("-" * 40)
