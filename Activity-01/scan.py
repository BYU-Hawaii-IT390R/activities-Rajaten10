...
def scan_txt_files(directory, min_size=0):
    ...
    filtered_files = [f for f in txt_files if f.stat().st_size / 1024 >= min_size]

    print(f"\nScanning: {directory.resolve()}")
    print(f"Found {len(filtered_files)} text files larger than {min_size} KB:\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-size", type=float, default=0, help="Only include files larger than this size (in KB)")
